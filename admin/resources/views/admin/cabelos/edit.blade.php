...
@section('title', 'Editando cor do cabelo')

@section('content')
    <br>
        <div class="card card-primary card-outline">
            <div class="card-header">
                Editando cor do cabelo: {{ $item->descricao }}
            </div>
            <div class="card-body">
                <form action="{{ route('cabelo.update', $item->id) }}" method="post">
                    @csrf
                    @method('PUT')
                ...
                <form id="delete-form" action="{{ route('cabelo.delete', $item->id) }}" method="POST" style="display: none;">
                    @csrf
                    @method('DELETE')
                </form>
            </div>
